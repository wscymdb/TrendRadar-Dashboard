...
const HeaderBar: React.FC<HeaderBarProps> = (props) => {
  ...
  return (
    <TooltipProvider delayDuration={150}>
      <header className="relative flex flex-col gap-3.5 p-4 lg:p-5 rounded-2xl bg-white/90 border border-zinc-200/90 text-zinc-900 shadow-sm backdrop-blur-xl dark:bg-zinc-950/80 dark:border-zinc-800/80 dark:text-zinc-100 dark:shadow-2xl transition-colors">
        {/* ================= 上层：主标题区与右侧控制区 ================= */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-zinc-100 dark:border-zinc-800/60">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-10 h-10 rounded-xl bg-zinc-100 border border-zinc-200 text-zinc-800 dark:bg-zinc-900 dark:border-zinc-800 dark:text-zinc-100 shadow-inner transition-colors shrink-0">
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                <span
                  className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium border ${healthBadgeColor}`}
                >
                  <span className={`w-1.5 h-1.5 rounded-full ${pulseDotColor} animate-ping`} />
                  <span>{healthText}</span>
                </span>
                {overview?.isDocker && (
                  <span className="inline-flex items-center gap-1 text-[11px] px-2 py-0.5 rounded bg-zinc-100 text-zinc-600 border border-zinc-200 dark:bg-zinc-800/60 dark:text-zinc-300 dark:border-zinc-700/50">
                    <Box className="w-3 h-3" /> Docker 容器
                  </span>
                )}
              </div>
        ...
        {/* ================= 下层：服务器环境元数据状态条 (响应式4列网格) ================= */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2.5 text-xs font-mono">
          {/* 1. 节点名称 */}
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-zinc-50 border border-zinc-200/70 dark:bg-zinc-900/40 dark:border-zinc-800/50 text-zinc-700 dark:text-zinc-300 transition-colors">
            <Server className="w-3.5 h-3.5 text-zinc-400 shrink-0" />
            <span className="text-zinc-400 dark:text-zinc-500 shrink-0">节点:</span>
            <span className="font-semibold text-zinc-900 dark:text-zinc-100 truncate" title={overview?.hostname}>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-zinc-50 border border-zinc-200/70 dark:bg-zinc-900/40 dark:border-zinc-800/50 text-zinc-700 dark:text-zinc-300 transition-colors">
            ...
            <span className="text-zinc-400 dark:text-zinc-500 shrink-0">系统:</span>
            <span className="font-semibold text-zinc-900 dark:text-zinc-100 truncate" title={overview?.os}>
              {overview?.os || 'Linux'}
            </span>
          </div>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-zinc-50 border border-zinc-200/70 dark:bg-zinc-900/40 dark:border-zinc-800/50 text-zinc-700 dark:text-zinc-300 transition-colors">
            ...
            <span className="text-zinc-400 dark:text-zinc-500 shrink-0">CPU:</span>
            <span className="font-semibold text-zinc-900 dark:text-zinc-100 truncate">
              {overview?.architecture || 'x64'} ({metrics?.cpu.coreCount || overview?.cpuCount || 1}核)
            </span>
          </div>

          {/* 4. 连续稳定运行时间 */}
          <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-emerald-50/60 border border-emerald-200/70 dark:bg-emerald-950/20 dark:border-emerald-500/20 text-emerald-800 dark:text-emerald-300 transition-colors">
            <Clock className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
            <span className="text-emerald-600/80 dark:text-emerald-400/70 shrink-0">已运行:</span>
            <span className="font-semibold text-emerald-700 dark:text-emerald-300 truncate">
              {formatSecondsToUptime(localUptime)}
            </span>
          </div>
        </div>
      </header>
