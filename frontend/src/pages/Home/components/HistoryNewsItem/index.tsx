...
interface HistoryNewsItemProps {
  item: NewsItem;
  index: number;
...
export const HistoryNewsItem: React.FC<HistoryNewsItemProps> = (props) => {
  const { item, index, copiedId, onCopy } = props;
  ...
  return (
    <div className="group flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 transition-colors hover:bg-zinc-50/80 dark:hover:bg-zinc-800/40">
      ...
      <div className="flex items-start sm:items-center gap-3.5 min-w-0 flex-1">
        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-md bg-zinc-100 font-mono text-xs font-semibold text-zinc-500 dark:bg-zinc-800 dark:text-zinc-400">
          {index + 1}
        </span>
      ...
      <div className="flex items-center gap-3 shrink-0 self-end sm:self-center pl-9 sm:pl-0">
        ...
        <Badge
          variant="secondary"
          className="gap-1 text-[11px] font-mono px-2 py-0.5 bg-zinc-100 text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300"
        >
          <Clock className="h-3 w-3 text-zinc-400" />
          <span>{item.duration || `在榜 ${item.occurrenceCount} 次`}</span>
        </Badge>
